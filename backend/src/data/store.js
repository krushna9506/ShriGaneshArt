...
/** Generate sequential SGA-XXXX order number */
const nextOrderNumber = () => {
  const num = orders.length + 1;
  return `SGA-${String(num).padStart(4, '0')}`;
};
...
export async function saveModels() {
  ...
  try {
    ...
    console.log('[Postgres Sync] Saved models to Neon.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Failed to save models:', err.message);
  }
}
...
export async function saveOrders() {
  try {
    ...
    console.log('[Store] Saved orders to orders.json');
  } catch (err) {
  ...
  try {
    ...
    await pool.query('COMMIT');
    console.log('[Postgres Sync] Saved orders to Neon.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Failed to save orders:', err.message);
  }
}
...
export async function savePayments() {
  try {
    ...
    console.log('[Store] Saved payments to payments.json');
  } catch (err) {
  ...
  try {
    ...
    await pool.query('COMMIT');
    console.log('[Postgres Sync] Saved payments to Neon.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Failed to save payments:', err.message);
  }
}
...
export async function saveCustomers() {
  try {
    ...
    console.log('[Store] Saved customers to customers.json');
  } catch (err) {
  ...
  try {
    ...
    await pool.query('COMMIT');
    console.log('[Postgres Sync] Saved customers to Neon.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Failed to save customers:', err.message);
  }
}
...
export async function saveInvoices() {
  ...
  try {
    ...
    await pool.query('COMMIT');
    console.log('[Postgres Sync] Saved invoices to Neon.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Failed to save invoices:', err.message);
  }
}
...
function loadModels() {
  try {
    if (fs.existsSync(modelsJsonPath)) {
      ...
      models.length = 0;
      models.push(...data);
      console.log(`[Store] Loaded ${models.length} models from models.json`);
      return;
    }
  } catch (err) {
    console.error('[Store] Failed to load models from JSON:', err);
  }
  console.log('[Store] Using default fallback models');
  models.length = 0;
...
export function loadStoreData() {
  ...
  try {
    if (fs.existsSync(ordersJsonPath)) {
      ...
      orders.length = 0;
      orders.push(...data);
      console.log(`[Store] Loaded ${orders.length} orders from orders.json`);
    }
  } catch (err) {
  ...
  try {
    if (fs.existsSync(paymentsJsonPath)) {
      ...
      payments.length = 0;
      payments.push(...data);
      console.log(`[Store] Loaded ${payments.length} payments from payments.json`);
    }
  } catch (err) {
  ...
  try {
    if (fs.existsSync(customersJsonPath)) {
      ...
      customers.length = 0;
      customers.push(...data);
      console.log(`[Store] Loaded ${customers.length} customers from customers.json`);
    }
  } catch (err) {
...
async function seedModelsToPostgres() {
  if (!process.env.DATABASE_URL) return;
  try {
    console.log('[Postgres Sync] Seeding initial models to Neon...');
    await pool.query('BEGIN');
    ...
    await pool.query('COMMIT');
    console.log('[Postgres Sync] Seeding complete.');
  } catch (err) {
    try { await pool.query('ROLLBACK'); } catch {}
    console.error('[Postgres Sync] Seeding failed:', err.message);
  }
}

export async function syncFromPostgres() {
  if (!process.env.DATABASE_URL) {
    console.warn('[Postgres Sync] No DATABASE_URL. Cloud sync skipped.');
    return;
  }
  try {
    console.log('[Postgres Sync] Hydrating in-memory arrays from Neon...');
    ...
    if (modelsRes.rows.length > 0) {
      ...
      console.log(`[Postgres Sync] Loaded ${models.length} models from Neon.`);
    } else {
      console.log('[Postgres Sync] pg_models empty — seeding defaults...');
      await seedModelsToPostgres();
      const reload = await pool.query('SELECT * FROM pg_models ORDER BY id ASC');
      models.length = 0;
      models.push(...reload.rows.map(row => ({
        id: row.id, code: row.code, name: row.name, size: row.size,
        material: row.material, price: Number(row.price),
        totalStock: row.total_stock, soldStock: row.sold_stock,
        remainingStock: row.remaining_stock, active: row.active
      })));
    }

    // 2. Customers
    const custRes = await pool.query('SELECT * FROM pg_customers ORDER BY id ASC');
    customers.length = 0;
    customers.push(...custRes.rows.map(row => ({
      id: Number(row.id), name: row.name, mobile: row.mobile,
      address: row.address, city: row.city, notes: row.notes
    })));
    console.log(`[Postgres Sync] Loaded ${customers.length} customers from Neon.`);

    // 3. Orders
    const ordersRes = await pool.query('SELECT * FROM pg_orders ORDER BY id ASC');
    orders.length = 0;
    ...
    console.log(`[Postgres Sync] Loaded ${orders.length} orders from Neon.`);

    // 4. Payments
    const paymentsRes = await pool.query('SELECT * FROM pg_payments ORDER BY id ASC');
    payments.length = 0;
    ...
    // 5. Invoices
    const invoicesRes = await pool.query('SELECT * FROM pg_invoices ORDER BY invoice_number ASC');
    invoices.length = 0;
