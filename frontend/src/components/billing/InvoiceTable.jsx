    ...
    <div className="overflow-x-auto rounded-xl border border-rose-800 bg-white">
      <table className="min-w-full text-left border-collapse">
        <thead className="bg-rose-100 text-rose-955 font-bold border-b border-rose-800">
          <tr>
            <th className="px-2 py-1.5 border-r border-rose-800 text-center w-10">अ.क्र.<br />Sr. No.</th>
            <th className="px-2.5 py-1.5 border-r border-rose-800">मालाचा तपशिल / Model Details & Particulars</th>
            <th className="px-2 py-1.5 border-r border-rose-800 text-center w-12">नग<br />Qty</th>
            <th className="px-2 py-1.5 border-r border-rose-800 text-right w-16">दर<br />Rate</th>
            <th className="px-2 py-1.5 text-right w-20">रुपये<br />Amount (₹)</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, index) => (
            <tr key={`${item.model_name}-${index}`} className="border-b border-rose-200 text-slate-800 font-bold hover:bg-rose-50/20">
              ...
              <td className="px-2.5 py-1 border-r border-rose-200">
